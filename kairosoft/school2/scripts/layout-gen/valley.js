...
/* ── 1) 分區設定（zone-first）：key = 街廓左上角 'r0,c0' ───────────────────────
   ...
   ★ 分區是第一公民、鋪面是分區的表達。**每個街廓都宣告 mat**（沒有無主的街道），
     ...
     R3   西溪動物園   西高地運動臺   南教學西棟   南教學核心   南教學東翼   東緣坡
     R4   南運動園區西  南運動園區東   南校舍南棟   南池畔校舍   開羅廣場     東南丘
     R5   南緣步道     南緣步道      南緣步道     南門玄關     南緣步道     星空台

   四塊語意各自成塊：
     · 走廊（室內）＝舊校舍臺地 → 谷底新市街 → 南校區，**13 個街廓連成一整片**
       （'5,5'→'5,10'→'10,10'→'15,10'→'20,10'→'20,15'→'15,15'→'15,20' 一路相鄰）
     · 水泥地（運動）＝西高地運動臺（e3 高台）＋它南麓的 R4C0／R4C1，一塊 L 形
     · 草地（自然）＝西溪綠廊北段／南緣步道／東緣崖坡→南池東岸→東南丘，三條地形帶
     · 道路（對外）＝校門玄關＋東緣廊帶＋東峽谷（北門）與 南門玄關（南門）

   ★ 為什麼運動園區在西南、教學區佔掉整個南平原中央：**溪谷的 e1 只有 3 列深**
     （谷底 r10–r12、南平原 r16–r18、r22–r23），一個 4×4 判定窗口裡塞得下三座 2×2 的
     街廓**全圖只有兩個**（'20,0' 與 '20,10'，因為它們各有 4 列可用）。
     「熱血」（足球場＋體育館＋游泳池）與「藝術」（美術室＋音樂室＋多功能室）各要一個，
     所以一個給運動、一個給教學 —— 分區配置是被這個幾何事實決定的，不是先畫好再套。
...
const ZONES = {
    ...
    '5,0': { name: '西溪農牧', fac: ['pig', 'cow', 'farm', 'chicken', 'rabbit', 'duck', 'mole', 'weather'], green: 'grass', mat: 'grass' },  // 12 格
    /* 西溪動物園（12 格）：長頸鹿＋大象＋圖騰柱＋銅像 → 青空／非洲；
       稀有動物（熊貓／無尾熊）都不是任何景點的材料，擺在綠廊最深處＝後期開發。 */
    '10,0': { name: '西溪動物園', fac: ['giraffe', 'elephant', 'totem', 'statue_br', 'panda', 'koala', 'statue_gold', 'well'], green: 'grass', stage: 4, mat: 'grass' },
    ...
    /* rows0–3 c5–8 幾乎全是 e2 台地（只有 X5/Y19 一格 e1）→ 交給 fillPlateau。
       ★ 這一版從「北緣台地花園」改成**北台地教學棟**：九間教室要成塊，而台地不吃街廓
         容量（parcels 只切 e1），這片 22 格的 e2 台地是全圖唯一排得下四座 2×2 的空地。 */
    '0,5': { name: '北台地教學棟', fac: ['bench'], green: 'grass', stage: 2, mat: 'wood_path' },
    ...
    /* ── 南運動園區（水泥地）：西高地運動臺（e3 高台）＋它南麓的 R4C0／R4C1 ───────
       六座 2×2（操場／棒球場／足球場／體育館／道場／游泳池）要 6 個象限：
       '20,0' 給 3（熱血三件套同框）、'20,5' 給 2，第 6 座（道場）與棒球場走高台 PREPLACE。 */
    // 西溪運動場（12 格／2 象限）：綠廊南段整段交給運動園區 —— 崖坡就在街廓裡（c3 整列
    // 是 e2 坡道），黃昏（棒球場＋崖坡＋樹林）在這裡零妥協成立
    '15,0': { name: '西溪運動場', fac: ['club', 'trampoline', 'locker', 'vending'], green: 'woods', stage: 3, mat: 'concrete' },
    // 西高地運動臺（e3，19 格非坡格）：棒球場＋櫻花走 PREPLACE（fillPlateau 只吃 1×1）
    '15,5': { name: '西高地運動臺', fac: [], green: 'grass', stage: 3, mat: 'concrete' },
    // 運動園區北（6 格／1 象限）：夾在西高地與南校區之間的谷口，道場與社團帶
    '15,10': { name: '南運動園區北', fac: ['dojo', 'club', 'trampoline', 'locker'], green: 'woods', stage: 3, mat: 'concrete' },
    '20,0': { name: '南運動園區西', fac: ['locker', 'club', 'trampoline'], green: 'woods', stage: 4, mat: 'concrete' },              // 14 格／3 象限
    '20,5': { name: '南運動園區東', fac: ['field', 'basketball', 'club', 'trampoline', 'locker'], green: 'woods', stage: 3, mat: 'concrete' }, // 8 格／2 象限

    /* ── 南校區（走廊）：坡道帶以南的新校舍，四個街廓連成一大塊 ───────────────── */
    '15,15': { name: '南教學核心', fac: ['art', 'career', 'broadcast', 'locker'], green: 'flower', stage: 3, mat: 'wood_path' },   // 12 格／2 象限
    '15,20': { name: '南教學東翼', fac: ['multi_room', 'game_corner', 'vending', 'bench', 'locker'], green: 'flower', stage: 3, mat: 'wood_path' }, // 12 格／2 象限
    // 南校舍南棟（14 格／3 象限）＝全圖唯二「一個窗口塞得下三座 2×2」的街廓 → 藝術三件套
    '20,10': { name: '南校舍南棟', fac: ['home_ec', 'nurse', 'toilet', 'water', 'career'], green: 'flower', stage: 3, mat: 'wood_path' },
    // 南池畔校舍（12 格）：原生南池 10 格就在街廓中央 → 水岸／清爽（映山紅＋草地＋長椅）
    '20,15': { name: '南池畔校舍', fac: ['well', 'toilet', 'bench', 'incinerator'], green: 'azalea', stage: 3, mat: 'wood_path' },

    /* ── 南池東岸與東南丘（草地）：東緣崖坡一路接下來的自然帶 ───────────────── */
...
const PREPLACE = [
    ...
    /* ①c 西高地運動臺（e3，19 格非坡格）＝「夕陽下的高台棒球場」。
          棒球場是 2×2，而 fillPlateau 逐格放置、只吃 1×1 —— 高台上的大型球場只能預放。
          西高地四緣天然全是崖坡，窗口 X17–20/Y20–23 一次框住棒球場＋崖坡＋櫻花 → 黃昏。
          ※ 台地上的街廓（'15,5'）沒有任何 e1 空地 → parcels() 不會把它列進街廓清單，
            所以 design2 **看不到這個窗口**，它照樣會在運動園區另蓋一座棒球場。
            這是共用機具的既有行為（不改機具），結果是全鎮有兩座棒球場 —— 谷底一座、
            高台一座，剛好對得上「高台是黃昏景點的舞台」的敘事，照實記在頁面上。 */
    { t: 'baseball', r: 15, c: 5 },     // 2×2 → X17–18/Y21–22（e3 非坡格）
    ...
    /* ② 校舍規模（硬指標）：**教室 9 間（1–3 年級各 3 班）＋ 辦公室 2 間（遊戲上限）**。
          兩者都不是任何景點的材料，景點骨架沒有理由蓋它們，輪到分區填充時象限早被吃光
          （舊版實測整張圖只剩原生那 1 間教室 ＋ 1 間辦公室）。所以規劃順序倒過來：
          **先把 9×4 ＋ 2×4 ＝ 44 格釘好，再去排景點。**

          ★ 溪谷的解法是「教室全部上台地」。谷底 e1 只有 3 列深、一個 4×4 窗口塞得下
            三座 2×2 的街廓全圖只有兩個 —— 在 e1 釘 7 座 2×2 等於直接砍掉七個景點窗口。
            而台地（e2／e3）**根本不進 parcels()**（街廓只從 e1 空地切），釘在台地上
            一格景點容量都不吃，代價只是少了幾棟 fillPlateau 的 1×1 小設施。
            於是把北緣那片 e2 台地整片改成「北台地教學棟」，與 e3 的北高地舊教室
            隔一道 c11 坡道相望 —— 九間教室裡有七間集中在北半部這一大塊教學台地，
            **成塊、不散落**，也沒有一間掉進農牧或運動分區。

          配置（9 間教室）：
            北高地舊教室 e3   ×2：原生教室 X3–4/Y12–13 ＋ 新建 X2–3/Y14–15
            北台地教學棟 e2   ×4：X2–3 三連棟（Y22–23／Y20–21／Y18–19）＋ X4–5/Y22–23
            台地教學棟南翼 e2 ×1：X6–7/Y22–23（西中台地帶北端，接北台地的同一片 e2）
            舊校舍臺地肩 e2   ×1：X6–7/Y13–14（e1↔e3 之間的換乘平台，舊校舍的西廂）
            南教學東翼 e1     ×1：X19–20/Y4–5（南校區唯一一間，谷底也要有教室可上課）
          辦公室 2 間：原生 X9–10/Y11–12 ＋ 新建 X8–9/Y13–14（緊貼原生辦公室西側，
            兩間並排成一塊職員室，落在「選舉」窗口內、也不動舊校舍走廊）。 */
    { t: 'class', r: 0, c: 12 },        // 2×2 → X2–3/Y14–15（e3 高地，原生教室的西鄰）
    /* 北台地教學棟（e2，r0–r3 c4–c10）：r0–r1 兩列 14 格剛好排下三連棟，
       留 c10 一縱列當步道接 c11 的 e3 坡道（往北高地舊教室）與 c3 的 e2 坡道（往西谷）。 */
    { t: 'class', r: 0, c: 4 },         // 2×2 → X2–3/Y22–23
    { t: 'class', r: 0, c: 6 },         // 2×2 → X2–3/Y20–21
    { t: 'class', r: 0, c: 8 },         // 2×2 → X2–3/Y18–19（東鄰留 c10 步道）
    // r2–r3 c4–c5：台地西緣，靠 c3 坡道；c6–c7 留給步道與原生長椅／銅像（清爽的窗內材料）
    { t: 'class', r: 2, c: 4 },         // 2×2 → X4–5/Y22–23
    // 南翼：西中台地帶北端 r4–r5 c4–c5，與上面那間隔著 c3 坡道走廊連成一條教學台地
    { t: 'class', r: 4, c: 4 },         // 2×2 → X6–7/Y22–23
    /* 舊校舍臺地肩西廂（e2 換乘平台 r4–r5 c10–c14）：北接 c11–c12 的 e3 坡道（舊校舍），
       南接 c10–c11 的 e2 坡道（下谷底）。釘 c13–c14 兩欄，c10–c12 全留步道。 */
    { t: 'class', r: 4, c: 13 },        // 2×2 → X6–7/Y13–14
    { t: 'class', r: 17, c: 22 },       // 2×2 → X19–20/Y4–5
    /* 辦公室 #2（e3 舊校舍草坪東半）：原生辦公室在 X9–10/Y11–12，這一座貼著它的西側，
       兩間並排＝一整塊職員室。草坪剩下的 X9/Y15–19 與 X10/Y18–20 仍是草地
       （力量／菜園的材料），而且是 X9/Y20 擦格坡道上來後唯一的一條草坪走廊，
       所以只吃 r6–r7 兩列的東端 4 格，一格走廊都不擋。 */
    { t: 'office', r: 6, c: 13 }        // 2×2 → X8–9/Y13–14
];
...
const res = D.build(B.spotOrder(), {
    ...
    prepare(g) {
        ...
        /* 第二座校門要在 parcels() **之前**加。原本只有北門(X2/Y5–6)，而它在 e3 台地上、
           要繞兩層坡才下得到南平原。南門開在下緣 X27/Y8–9，正對 c19 縱幹道脊椎。
           ※ 放在 fill 之後會失敗：南門那兩格落在街廓 '25,15' 裡，景點骨架與 4×4 補位
             早就把 X27/Y9 佔掉了（addGate 的「兩格都要是空地」直接靜默回 false，
             實測整張圖只剩北門一座）。加在 prepare 裡還有額外好處 ——
             parcels() 是用可達性決定哪些格可蓋，南平原多一座門就多一批 slots。 */
        if (!B.addGate(g, 25, 18, 'gate_h')) throw new Error('南門加不上去 (25,18)');
    }
});
const g = res.g;
...
/* 4) 分區填充（西溪綠廊、谷底新市街、南平原）。南門已在 prepare() 加好（見上）。 */
...
B.fillPlateau(g, [
    // 北台地教學棟：四座 2×2 教室已預放，這裡只補剩下的零星格（步道鋪走廊＝校舍語意）
    { name: '北台地教學棟', rows: [0, 3], cols: [4, 10], fac: ['bench', 'statue_br', 'board', 'locker'], green: 'sakura', walkMat: 'wood_path' },
    ...
    // 西中台地帶：北端 r4–r5 已讓給教學棟南翼（第 5 間教室），這裡從 r6 起算
    { name: '西中台地帶', rows: [6, 13], cols: [4, 6], fac: ['statue_br', 'board', 'water', 'locker', 'bench', 'weather'], green: 'grass', walkMat: 'grass' },
