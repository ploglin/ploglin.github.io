...
/* 中央教學（走廊）：R2 整列是全圖第二大片連續空地，新校舍主體。
   ※ 這兩個街廓的 4×4＝4 個象限**全部釘給教室**（見 1d 的 9 間硬指標），
     fac 因此留空 —— 再列 music／art／home_ec 只會每輪印一串「無處可放」。
     專科教室改由西教學／舊校舍／中央生活三塊走廊分區承接。 */
zone(['10,0'], { name: '中央教學', fac: [], green: 'flower', decor: true, stage: 2, mat: 'wood_path' });
zone(['10,10'], { name: '中央教學', fac: [], green: 'flower', decor: true, stage: 3, mat: 'wood_path' });
...
/* 1d) 教室要**先卡位**。教室是 2×2，而且**不是任何景點的材料** ——
      ...
      實測整張圖只會剩 1 間（原生那間）。
      ★ 硬指標（使用者實機定案）：**每張地圖固定 9 間教室（1–3 年級各 3 班）
        ＋ 2 間辦公室（遊戲上限）**。這 9×4 ＋ 2×4 ＝ 44 格是校舍規模的下限，
        不是「排完景點剩多少算多少」—— 教室與辦公室都不是任何景點的材料，
        正因如此最容易在排景點時被擠光（舊版就只剩 7 間，verify 直接 FAIL）。
      所以規劃順序是：**先把教室與辦公室釘死（preplace），再去排景點**。
      冬郵的 9 間 ＝ 中央教學兩個街廓各釘滿 4 個象限（8 間）＋ 舊校舍的原生教室
      （r9–10/c18–19）1 間。原生那間本來就在走廊分區裡，語意一致，不用重蓋。
      ※ preplace 跑在 layRoads／prepare 之後、parcels 之前，所以這些格子不會被切成
        街廓 slot，景點骨架會自動繞開；每一棟都貼著街廓外緣的街道，不會被圍死。
      ※ 試過的替代點位（都退回）：
        - 舊校舍 (10,20) 那塊 2×2 純走廊 → 把**原生公告欄 X12/Y5 圍死**（verify FAIL）。
        - 舊校舍 (6,20) → 切斷東緣高地坡道，百葉箱＋小農場×3 全部走不到（verify FAIL）。
        中央教學的兩個空象限則零代價：原本只放了花壇／草地與 1 格走廊。 */
const PREPLACE = [
    // 中央教學西棟（X12–X15 / Y27–Y24）：整個街廓 4 個象限全是教室
    { t: 'class', r: 10, c: 0 }, { t: 'class', r: 10, c: 2 },
    { t: 'class', r: 12, c: 0 }, { t: 'class', r: 12, c: 2 },
    // 中央教學東棟（X12–X15 / Y17–Y14）：同樣 4 個象限全是教室
    { t: 'class', r: 10, c: 10 }, { t: 'class', r: 10, c: 12 },
    { t: 'class', r: 12, c: 10 }, { t: 'class', r: 12, c: 12 },
