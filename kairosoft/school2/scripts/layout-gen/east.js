/* 冬郵小鎮完美佈局 — 設定檔。實際流程都在 builder.js（城鎮無關）。
   ...
   - 東南角有一塊陸地被水塘圍住。實機確認**水塘可以被建設覆蓋破壞、變回平地**，
     所以那不是死地 —— 但圍島的水塘記在 e2、島上平地是 e1，**鑿掉鋪路也差一階走不過去**，
     要把 2 格 e2 水塘擦成「空地」讓遊戲自動判成斜坡才接得上（見 prepare 的登島坡道）。
   - **不破壞原生地形換動線**：分區優先重排後，舊版那三組 breakTerrain（鑿 4 格水塘
     ＋砍 3 棵原生植栽）全部被守衛退回（動線總分沒變好），整段已移除。
     全圖只破壞 2 格水塘＝登島坡道，原生樹木一棵都沒砍。
   - 稀有動物（熊貓／無尾熊／鱷魚）擺在接通後的湖心島＝「後期動物島」，
     跟階段敘事一致（基礎農牧留西側）；這幾種都不是任何景點的材料，搬島零風險。
   - 東緣兩間原生養豬小屋**只留一間**：北邊那間四面是水塘與地圖邊界，怎麼蓋都走不到，
     拆南邊那間改鋪草地當參道（見 prepare 末尾）。 */
...
const res = D.build(B.spotOrder(), {
    ...
    prepare(g) {
        ...
        /* ★ 東緣兩間原生養豬小屋:拆一間讓另一間走得到。
           兩間並排在地圖最東欄（X13/Y2 與 X14/Y2，都是 e2）:
             北邊 X13/Y2 —— 北是水塘(e2)、西是水塘(e2)、東是地圖邊界、南是另一間豬舍
                            → **四面都不是出入口，怎麼蓋都走不到。**
             南邊 X14/Y2 —— 南邊接 X15/Y2 的草坡(e2，同高度) → 有門。

           ※ 這一間長年被包圍判定的第二個盲點遮住:舊版把**同型別相鄰的格子 flood fill
             成「一棟」**,所以兩間並排的豬舍只要其中一間有門就一起過關。
             engine 改成按 w×h 逐棟判定後才現形（跟 footprint 檢查同一把尺）。

           要給北邊那間開門只剩「再鑿一格水塘」,但水塘預算已全數用在湖心島登島坡道
           (2/2，那 2 格是不鑿就走不到 15 格陸地的必要工程) —— 為一間小農舍破壞第 3 格
           不划算。依管線既有原則「怎麼蓋都走不到的建築會拆遷」(湖岸小鎮的原生鱷魚池
           就是這樣處理的),拆掉一間。

           **拆南邊那間、把那格改鋪草地**(不是拆北邊那間):
             X15/Y2 草坡(通) → X14/Y2 草地(新) → X13/Y2 豬舍  ← 同高度，走得通
           兩種做法都是「少一間豬舍」,但這樣**留下的是原地保留的原生建築＋一條真的參道**,
           而拆北邊那間只會多出一格死景觀。實測也較好:143 棟 vs 141 棟、死路支線 21% vs 22%。
           草地在這裡可通行且是菜園／力量／清爽的材料,只會多景點不會少;
           「特盛」(養豬小屋＋養牛小屋＋餐廳)用的是西南水岸那一組,不受影響。 */
        if (g[12][25].type !== 'pig') throw new Error('拆遷落點不是養豬小屋：12,25');
        g[12][25] = { type: 'grass', elevation: g[12][25].elevation };
    }
});
const g = res.g;
