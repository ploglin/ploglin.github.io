...
/* 7b) 環化 pass（動線流暢優先於景點配置）：孤立通行格歸零、2–4 格的設計性死路支線
      接回成環或整條收成綠地；門前水泥廣場／幹道脊椎／校門門面／斜坡一律豁免。
      開關與參數在 towns.js 的 flow —— 本鎮另外點名豁免西北高地的 16 格原生草坡
      （懸崖圍死的景觀窗，水岸／力量的窗內材料，一格都不准動）。
      崖下行政區的 r13–14,c22 動線不必列：它是宇宙火箭／茶室的唯一門面，
      「被包圍不可變多」的守衛天生擋住收枝。 */
