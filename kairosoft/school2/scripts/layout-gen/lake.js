/* 湖岸小鎮完美佈局 — 設定檔。實際流程都在 builder.js（城鎮無關）。
   ...
   - **游泳池朝向未實機確認**：preset 只記了 (16,15) 一格（pool 註冊為 2×2）。含它的合法
     2×2 只有 (16–17,15–16)，其餘方向都會壓到山中湖或走廊。設計上 **r15–17 × c14–16
     整片不排任何新建**，體育館排在它東南側，所以朝向不論往哪邊長，熱血的 4×4 窗口
     與動線都不受影響。頁面上照實標「待實機確認」。
   - **埋伏的巨石靠玩家放置**：本圖沒有原生 rock。模擬器裡 rock 是非 hidden 的環境筆刷，
     視為可建，所以照 29/29 實作；若實機證實巨石不能自己放，上限就是 28/29。
     頁面誠實說明段有註明這個假設。 */
...
const PREPLACE = [
    ...
    /* E3. 泳池體育館區 W(15,14)＋澀谷借位窗 W(17,12)：
         熱血＝原生泳池 (16,15)＋體育館 2×2 (17–18,16–17)＋足球場 (17,15)
         澀谷＝便利商店 (20,12–13)＋足球場 (17,15)＋社團教室② (18,15)
       r16 的 c16–18 三格北面全是山中湖、唯一出入口在 (16,19)，所以那排留給填充自己決定。 */
    { t: 'soccer', r: 17, c: 15 }, { t: 'club', r: 18, c: 15 }, { t: 'gym', r: 17, c: 16 },
