...
量測時間：2026-08-04，B0「地基與上鎖」批次完成時（**B1「資料庫擴充」已於 `2ce3568` 落地**，
下表數字含 B1 新增的 `db/terms`、`db/achievements` 兩頁）。
...
> B1 與 B0 是並行執行的，B1 已先落地（`2ce3568`：db 12→14 分類、9 表換上逐項精確資料、四表欄位升級），
> 所以本檔的數字**已含 B1 的結果**——sim↔db 的逐列比對在 B1 改完 `db/data.js` 之後仍是 0 處不同，
> 這正是 B0 想證明的事。後續 B3（db 加厚）會再動各 db 頁的字數，屆時重跑並更新「各頁可見字數」一節即可。
...
| 項目 | 基線 |
|---|---|
| 掃描頁面 | **174** 頁（`.html`，跳過 `.git`/`.idea`/`node_modules`/`assets`/`scripts`/`scratchpad`/`.github`） |
| 內部連結 | **1,780 條 0 壞**（頁面 1,207、靜態資源 573） |
| 外部連結 | 277 條（只計數，不連網） |
| app 狀態片段 | 8 條（`sim/#m=…` 分享碼，不驗 id） |
| 逸出 repo 的相對路徑 | 0 條 |
| 錨點 | **474 條**（同頁 464、跨頁 **10**）→ **0 壞**（另 1 筆列入已知問題，見下） |
| 同頁 id 重複 | 0 頁 |
| canonical | 171 / 174 頁有；**全部等於 `https://ploglin.cc/` ＋ repo 相對目錄** |
| og:url | 171 / 174 頁有；全部等於 canonical 應有值 |
...
- **跨頁錨點 10 條 0 壞** ✔ 完全吻合。
- **連結 484 → 實測 1,780 條**（頁面 1,207 ＋ 資源 573）。差距是計數基準不同，不是有連結消失：
  484 應該是某個子集（例如只算 `<a href>` 且去重後的目標數）。本檔的 1,780 是
