window.GAME_DB = {
  game: {
    id: 'mega-mall2', title: '都市大亨物語2',
    intro: [
      '《都市大亨物語2》的分數集中在店舖的 Rep（名聲）、Price（單價）與 Qual（品質）三個數字上，而拉高它們的手段就兩種：同層貼合的 combo，以及樓層相容加成。這個資料庫依此拆成三張表——43 組 combo、店舖分類、建築三大類。',
      '新手先看「建築三大類」弄清楚移動／通道、店舖、設施的差別：只有店舖吃樓層加成，設施要貼到店舖才傳得出數值，走道與交通工具只負責把顧客帶進來。接著用「店舖分類」認識五大類店舖各自的定位與代表店名，再挑 combo 動工。',
      '老手拿 combo 表做樓層規劃：先按「理想樓層」欄把 43 組分群，同一層盡量塞進理想樓層重疊的組合，加成才疊得起來；標「任意」的組合沒有樓層限制，留著補空位。表中英文店名對齊遊戲內顯示，方便直接對照實機介面。'
    ]
  },
  categories: [
    {
    ...
    },
    {
      ...
      intro: '店舖是商場的評分主體：會賣東西、產生營收，combo 與樓層相容的加成也都堆在店舖身上。本表把店舖依用途分成食品、電器、居家、美容時尚、娛樂五大類，再加一類設施。「代表店舖」欄同時列出英文與中文名，方便對著遊戲介面找；「備註」欄寫的是該類的實戰定位——居家類便宜、適合早期現金流，娛樂類造價高但加成也高，設施類則不吃樓層加成，要靠緊貼店舖把數值傳過去。',
      columns: ['分類', '代表店舖', '備註'],
      rows: [
    ...
    },
    {
      ...
      intro: '商場的可放置結構分三大類，這張表是理解佈局規則的前提。關鍵在「樓層加成」欄：只有店舖有樓層相容性，把對的店放到對的樓層才會多拿加成；移動／通道與設施都沒有，位置只影響動線與接觸。因此規劃順序是先用走道與交通工具把客流引到樓層深處，再把要衝分的店舖排成 combo，最後用設施填縫——設施沒接觸到店舖等於白蓋。',
      columns: ['類別', '說明', '樓層加成'],
      rows: [
