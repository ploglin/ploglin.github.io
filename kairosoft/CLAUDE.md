...
頁面是寫給玩家看的，**不是工作紀錄**。以下三種在 2026-08-10 全站清過一次，不要再長回來：

- **不寫製作過程**：「初版把西半部四個街廓全給了農牧」「我們第一版就踩了這個坑」「中途試過把教室釘在這裡」「規劃過程中我們試過三種分區配置」「2026 年 8 月追加：…」（callout 標題變成改版公告）。分界線是**講「這樣排會出什麼事」留著，講「我上一版排錯了」刪掉**——同一個實測數字兩種寫法通常都成立，把立場從回顧改成建議即可。
- **不外洩內部工具術語**：「自動排版」「景點骨架」「排版器」「候選清單」，以及「驗證的『沒有無中生有的新水塘』會失敗」這種整句在講 `layout-gen`／`verify.js` 的話。讀者不知道這些東西存在，要改寫成遊戲端的說法（「實機蓋不出新水塘」）。
- **不做版本對版本的比較**：「蓋錯區從 17 棟降到 12 棟」「百靈從 157 棟長到 188 棟」比的是本站前後兩版草稿。要留這個數字就改寫成**方案對方案**（「不這樣做 17 棟蓋錯區，這樣做剩 12 棟」）或**階段對階段**（「蓋到 157 棟時 7%，蓋滿 188 棟升到 20%」）。

**反過來，這些是可信度資產，不要一起砍**：「經程式逐格驗證」、db 頁的「常見誤解與實測註記」「本站標記存疑」、「一處刻意偏離／照實記錄，不改分區名圓場」。
